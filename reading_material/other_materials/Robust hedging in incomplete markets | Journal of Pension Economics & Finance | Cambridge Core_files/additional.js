...
/**
 *
 * AJAX FORM HANDLING FOR THE WEBSITE
 *
 */
...
var createAlertBox = function (form, alertElement, alertType, message) {
  ...
  if (message) {
    var alertBox = $('<div data-alert class="alert-box ' + alertSettings[alertType].boxClass + '"><div class="alert ' + alertSettings[alertType].messageClass + '">' + message + '</div><a href="#" class="close" tabindex="1">&times;</a></div>');
...
var submitAjaxForm = function (form, alertElement, callback) {
  ...
  $.ajax({
    ...
    success: function (resp) {
      ...
      createAlertBox(form, alertElement, alertBoxStatus, message);
