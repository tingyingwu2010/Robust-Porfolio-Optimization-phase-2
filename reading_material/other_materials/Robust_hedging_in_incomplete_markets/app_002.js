...
AOP.loadQtip = function () {
  ...
  $('.download-types li a, .social li a').each(function (i, e) {
    var $e = $(e);
...
AOP.styleInputElements = function (container) {
  ...
  if (container) {
    selector = $('input[type=checkbox], input[type=radio]', container).not('.styled');
  } else {
    selector = $('input[type=checkbox], input[type=radio]').not('.styled');
  }
...
AOP.loadGlobalNav = function () {
  ...
  if (!AOP.isReaderPage()) {
    try {
      globalNav(
        '//www.cambridge.org/tools/packages/cambridge_themes/global_navigation/v1.2',
        {},
        function (resp) {
...
AOP.validateOrcidId = function (orcidId, customErrorElSelector, callback) {
  ...
  $.ajax({
    ...
    var results = data && data.name && data.path;
    if (results) {
      customErrorEl.hide();
    ...
    customErrorEl.show();
    customErrorEl.parent().parent().addClass('error');
...
(function ($) {
  ...
  $('body').on('click', '.listing-citation-modal', function (e) {
    ...
    if (citationsLoaded === 'yes') {
      ...
      $.post(serviceUrl, {doi: doi, productType: productType, productId: productId}).done(function (response) {
        $loader.hide();
        if (response.success) {
  ...
  var initSecondaryMenuDropdowns = function (elementId, opts) {
    ...
    var dropdown = {
      ...
      // Hide the dropdown
      hide: function () {
        this.$el.css({ left: -9999, top: this.top }).attr('aria-hidden', 'true');
      },
      // Show the dropdown
      show: function () {
        this.$el.css({
    ...
    // Anything detected as mobile device, toggle dropdown visibility or load the requested link.
